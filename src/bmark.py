...
def regular_hash():
    ...
    processed = {
        x[0]: (x[3] - x[2]) / x[1] if x[3] and x[2] else None for x in raw_data
    }
    return processed
...
class TestCorrectness(TestCase):
    """Verify correctness of benchmarked code"""
    ...
    def test_regular_hash(self):
        ...
        self.assertEqual(
            {
                key: round(value, 7) if value else None
                for key, value in regular_hash().items()
            },
            processed_data,
            "Regular Hash is correct",
        )

    def test_numeric_regular_hash(self):
        """Verify numeric_regular_hash()"""

        self.maxDiff = None
        self.assertEqual(
            numeric_regular_hash(),
...
def main():
    ...
    print(
        "Regular Hash:\t\t",
        timeit(regular_hash, number=timed_iteration),
        "s\n",
        "Numeric Regular Hash:\t",
        timeit(numeric_regular_hash, number=timed_iteration),
        "s\n",
        sep="",
    )
